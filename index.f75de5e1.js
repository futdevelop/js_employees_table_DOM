...
function sortTable(columnIndex, ascSort) {
    const sortedRows = rows.sort((a, b)=>{
        ...
        if (!isNaN(aNum) && !isNaN(bNum)) {
            if (ascSort) return bNum - aNum;
            return aNum - bNum;
        } else {
            if (ascSort) return bText.localeCompare(aText);
...
for(let i = 0; i <= 4; i++){
    ...
    if (attr === "age" || attr === "salary") input.setAttribute("type", "number");
    ...
    if (attr === "office") {
        ...
        for(let j = 0; j < options.length; j++){
            ...
            const text = optionText.charAt(0).toUpperCase() + optionText.slice(1, option.length);
...
form.addEventListener("submit", (e)=>{
    ...
    for(const key in data){
        ...
        if (+data[key] > 100) td.textContent = formatSalary(data[key]);
        else td.textContent = data[key];
        tr.appendChild(td);
    }
    let isError = false;
    if (data.name.length < 4) isError = true;
    if (+data.age < 18 || +data.age > 90) isError = true;
    if (!isError) {
        tBody.appendChild(tr);
        pushNotification("Successfully added a new employee", "success");
    } else pushNotification("Unexpected error occured", "error");
    isError = false;
    form.reset();
});
